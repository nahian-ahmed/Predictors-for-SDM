...
def get_buffered_points (input_points):
    ...
    points_150 = add_buffer(input_points, 150)
    points_300 = add_buffer(input_points, 300)
    points_600 = add_buffer(input_points, 600)
    points_1200 = add_buffer(input_points, 1200)
    points_2400 = add_buffer(input_points, 2400)
        
    return points_75.merge(points_150).merge(points_300).merge(points_600) \
        .merge(points_1200).merge(points_2400)
...
def load_image_data (start_year, end_year):
    ...
    spring = ee.ImageCollection('users/JohnBKilbride/bird_species_2020/spring_nbr')
    summer_nbr = ee.ImageCollection('users/JohnBKilbride/bird_species_2020/summer_nbr')
    summer_b5 = ee.ImageCollection('users/JohnBKilbride/bird_species_2020/summer_b5')
    fall = ee.ImageCollection('users/JohnBKilbride/bird_species_2020/fall_nbr')
    
    # Compute the metrics required for each of the time-series
    spring = compute_spectral_metrics(spring, 'spring_nbr')
    summer_nbr = compute_spectral_metrics(summer_nbr, 'summer_nbr')
    summer_b5 = compute_spectral_metrics(summer_b5, 'summer_b5')
    fall = compute_spectral_metrics(fall, 'fall_nbr')
      
    # Combine into into one image for each year
    def join_images (year):
        
        # Cast the input year
        year = ee.Number(year)
           
        # Define the dates needed to filter the collection
        start_date = ee.Date.fromYMD(year, 1, 1)
        end_date = ee.Date.fromYMD(year, 12, 31)
            
        # Get the correct images
        spring_image = ee.Image(spring.filterDate(start_date, end_date).first())
        summer_nbr_image = ee.Image(summer_nbr.filterDate(start_date, end_date).first())
        summer_b5_image = ee.Image(summer_b5.filterDate(start_date, end_date).first())
        fall_image = ee.Image(fall.filterDate(start_date, end_date).first())
        
        # Join the images
        return spring_image.addBands(summer_nbr_image, None, True) \
            .addBands(summer_b5_image, None, True) \
            .addBands(fall_image, None, True) \
            .toFloat()    
     
    spectral_metrics  = ee.ImageCollection.fromImages(ee.List.sequence(start_year, end_year).map(join_images))
    
    # Combine the image collections into a single ee.ImageCollectiion with one image for each yearof the time-series
    all_metrics = compute_dem_metrics(spectral_metrics)
...
def generate_names():
    ...
    # Define these thangs
    seasons = ['spring_nbr', 'summer_nbr', 'summer_b5', 'fall_nbr']
...
#    # Load in EDB WETA data
#    weta_points = ee.FeatureCollection("users/JohnBKilbride/bird_species_2020/edb_weta_sample_points") \
#        .select(["Unique_Che","Year","Date","Latitude","Longitude"],
#                ["Unique_Checklist_ID","Year","Date","Latitude","Longitude"])
#    main(weta_points, start_sample_year, end_sample_year, 'bird_buffer_samples_')
    
    # Load in the EBird data
    oregon_2020_points = ee.FeatureCollection('users/JohnBKilbride/bird_species_2020/ebird_locations') \
        .select(["checklist_id","year"], ["Unique_Checklist_ID","Year"])
    main(oregon_2020_points, start_sample_year, end_sample_year, 'ebird_oregon_2020_')
