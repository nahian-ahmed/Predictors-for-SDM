...
def load_image_data (start_year, end_year):
    ...
    summer_nbr = ee.ImageCollection('users/JohnBKilbride/Bird_SDM_2025/oregon_interpolated_medoids')
...
def compute_dem_metrics (input_collection):
    ...
    ned_dem = ee.Algorithms.Terrain(ee.Image("USGS/3DEP/10m")).select(['elevation', 'slope', 'aspect'])
...
if __name__ == '__main__':
    ...
    oregon_2020_points = ee.FeatureCollection('users/JohnBKilbride/Bird_SDM_2025/OR2020_SpeciesRecords') \
        .select(["Unique_ID", "Year"], ["Unique_Checklist_ID","Year"])
